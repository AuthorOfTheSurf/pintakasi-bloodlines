...
export const MCP_INSTRUCTIONS: string[] = [
  ...
  `THE ECONOMY IS POOLED ($1 = ${ECONOMY.GP_PER_DOLLAR} GP): each fight's pot is TWO STAKES — one share from each bird (${realStake} GP a side at real stakes), NOT two entry fees. ` +
    ...
    ` No fight ever prints GP. Every REAL win also banks PINTAKASI QUALIFICATION POINTS (+${PINTAKASI.POINTS_FOR.real} a win; the discovery year pays ${PINTAKASI.POINTS_FOR.juvenile}) — each fight in the group counts on its own, so one night can bank ${PINTAKASI.POINTS_FOR.real * FIGHTS_PER_GROUP_BIRD}. That is how a bird earns its way into a championship, and since round 31 took hardcore off the daily card it is the ONLY way: winning a crown banks no points at all. The subsidy is LAND: it pays ONCE PER ENTRY at settle-up, on the TOTAL the bird actually risked, on a curve steeper than linear (a full card of ${FIGHTS_PER_GROUP_BIRD}: juvenile ${juvenileLand} LT · real ${realLand} LT; a short card pays less, honestly — it risked less; tournament fights mint per fight on their own, much steeper curve). Fighting UP into dearer company pays extra land PER GP RISKED, not merely more of it — a full real night pays ${(realLand / ECONOMY.REAL_ENTRY_FEE).toFixed(3)} LT/GP against the discovery year's ${(juvenileLand / ECONOMY.JUVENILE_ENTRY_FEE).toFixed(3)}, and the Majors are steeper again. (Round 34 briefly INVERTED this: land rounds to whole tokens with a floor of 1, and moving the juvenile night to ${ECONOMY.JUVENILE_ENTRY_FEE} GP tipped it over a rounding boundary, so the cheapest fight in the game paid the best land in it until LAND.FEE_PER_TOKEN was moved to match. If those two figures ever cross again, trust the figures and say so — do not repeat this sentence.) A bird that drew NOBODY earns none: land is for FIGHTING, not queueing. Land is also buyable (buy_land, $${usd(LAND.GP_PER_100_TOKENS)}/LT, capped at ${LAND.DAILY_BUY_CAP.toLocaleString()} LT/game-day) and NEVER sellable.`,
