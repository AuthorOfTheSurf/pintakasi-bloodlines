...
function createServer(farmId: string | null): McpServer {
  ...
  const server = new McpServer(
    { name: "pintakasi-bloodlines", version: "0.3.0" },
    {
      instructions: [
        ...
        "THE DAILY RITUAL: check_in once per game-day — it pays the GP drip ($10 = 800 GP) and 1 free gacha pull. Do it first thing.",
        ...
        "THE ECONOMY IS POOLED ($1 = 80 GP): both sides post the entry, winner takes the pot LESS a 2% rake to the Land Token stakers (round 22) — a 40 GP card pays 78.40. No fight prints GP; the rake MOVES, it isn't burned. Every win also banks PINTAKASI QUALIFICATION POINTS (real +1, hardcore +2, juvenile 0) — that is how a bird earns its way into a championship. The subsidy is LAND: both fighters earn Land Tokens scaled to the entry fee and slightly MORE than linearly (juvenile 1 LT · real 7 LT · hardcore 23 LT) — fighting UP into dearer company pays extra land. Unmatched birds earn none. Land is also buyable (buy_land, $0.01/LT, capped daily) and NEVER sellable.",
        ...
        "THE PINTAKASI — THE WEEKLY MAJORS (round 18): every THURSDAY (the week's last day), three blade championships crown specialists (Long Knife and Short Gaff always run; the middle blade rotates Short Knife / Long Gaff weekly). Single elimination in ONE day, winners healing to full between rounds; HARDCORE THROUGHOUT — every loser force-retires. Age 3+ and FREE TO ENTER (round 22) — but you must QUALIFY: a bird needs 3 qualification points, earned by winning on the daily card (real win +1, hardcore +2, the discovery year pays nothing). One bird per crown and up to 3 birds from one barn in the same championship (enter_pintakasi any day up to Thursday itself). The purse = the WHOLE week's juice pool, paid to the top (champion ~half; first-round losers zero GP) — but LAND pays the fallen hardest (elimination grants grow the earlier you fall, plus every tournament fight mints land on a steeper curve). The bracket is committee-seeded by qualification points → career earnings → wins → average figure, byes to the top seeds, and the field is PUBLIC all week (pintakasi_board). At 64 the committee bumps the weakest for a stronger newcomer. This is where champions — and breeding legends — are made: pitch it to the player when a bird hits age 3 strong.",
  ...
  server.registerTool(
    "check_in",
    {
      ...
      description:
        "The daily ritual, once per game-day: pays the GP drip (800 GP = $10) and grants 1 free gacha pull. Do this first thing each day.",
    },
  ...
  server.registerTool(
    "breed",
    {
      ...
      description:
        "Buy a cover: YOUR retired hen × a retired rooster — your own, or ANY farm's listed stud (browse_studs first). Costs 160 GP ($2, min AND max for now), which SPLITS: 8.00 GP to the land-staking pool, 76.00 to the fight-juice pool, 76.00 to the stud's owner. The hen's farm keeps the egg ('Egg of <mother>' — hatches next Hatch Friday). Covers are capped per rooster per week (14 public + 2 owner-reserved).",
      inputSchema: z.object({
  ...
  server.registerTool(
    "stake_land",
    {
      title: "Stake Land",
      description:
        "Stake Land Tokens into THE pool (single pool for now). Staked land earns a pro-rata share of every inflow — fight and claim rakes, gacha share, breed cut, land purchases — paid every day at the tick, which is where GP goes decimal. Stake as soon as you earn; unstake any time. STACK LAND: it may be worth real money one day ($1/LT is the dream), and it is never sellable either way.",
  ...
  server.registerTool(
    "enter_pintakasi",
    {
      ...
      description:
        "Register an age-3+ bird for one of THIS WEEK's three blade championships (the weekly Majors — crowns every Thursday). HARDCORE THROUGHOUT: every loser in the bracket is FORCE-RETIRED, so always confirm with the player first. FREE to enter, but the bird must hold 3 QUALIFICATION POINTS won on the daily card (real win +1, hardcore +2, juvenile 0); the purse = the week's whole juice pool, weighted to the TOP (first-round losers take zero GP) while LAND is weighted to the FALLEN (the earlier a bird goes out, the bigger its land grant — a first-round death is never a pure loss). Committee-seeded bracket by qualification points → career earnings → wins → average figure; at 64 entrants a stronger newcomer BUMPS the weakest. One bird per crown and up to 3 birds from your barn per championship; the field is PUBLIC — check pintakasi_board. Registrants fight normal cards all week except Thursday.",
      inputSchema: z.object({
        birdId: z.string(),
        format: z
          .enum(FORMAT_NAMES as [string, ...string[]])
  ...
  server.registerTool(
    "roll_gacha",
    {
      ...
      description:
        "One roll = 16 GP ($0.20, repriced round 22 — it was 80) and a farm may buy 5 a game-day; the free pull from check_in spends first. Always pays a rarity token (White/Green/Blue/Purple/Gold — prizes TBD) plus a Land Token. Blue, Purple, and Gold ALSO drop a MYSTERY EGG (random element, hidden sex, hatches next Hatch Friday) — announce it with fanfare. At this price an egg costs ~109 GP against a 160 GP cover, so rolling is now the CHEAP way to add a bird: 10% of the spend goes to the land stakers, 90% to the juice pool that pays the championships.",
    },
