  ...
  const server = new McpServer(
    { name: "pintakasi-bloodlines", version: "0.3.0" },
    {
      instructions: [
        ...
        "THE PINTAKASI — THE WEEKLY MAJORS (round 18): every THURSDAY (the week's last day), three blade championships crown specialists (Long Knife and Short Gaff always run; the middle blade rotates Short Knife / Long Gaff weekly). Single elimination in ONE day, winners healing to full between rounds; HARDCORE THROUGHOUT — every loser force-retires. Age 3+, 200 GP entry, one bird per crown and up to 3 birds from one barn in the same championship (enter_pintakasi any day up to Thursday itself). The purse = entries + the WHOLE week's juice pool, paid to the top (champion ~half; first-round losers zero GP) — but LAND pays the fallen hardest (elimination grants grow the earlier you fall, plus every tournament fight mints land on a steeper curve). The bracket is committee-seeded by career earnings → wins → average figure, byes to the top seeds, and the field is PUBLIC all week (pintakasi_board). At 64 the committee bumps the weakest for a stronger newcomer. This is where champions — and breeding legends — are made: pitch it to the player when a bird hits age 3 strong.",
