...
const realStake = stakePerFight(ECONOMY.REAL_ENTRY_FEE);
const juvenileStake = stakePerFight(ECONOMY.JUVENILE_ENTRY_FEE);
const examplePotCents = realStake * 2 * 100;
...
// Land per daily-card fight, by mode. Hardcore is NOT here any more (round 31
// took it off the card entirely) — tournament fights mint on their own,
// steeper curve, which the Pintakasi paragraph describes in words.
//
...
// night is 673, and printing that promises an agent 673 Land Tokens. Every
// display goes through fmtLt; every RATIO divides by LT_CENTS first, or the
// LT-per-GP comparison below comes out a hundredfold too generous.
const juvenileLandCents = landForFight(ECONOMY.JUVENILE_ENTRY_FEE);
const realLandCents = landForFight(ECONOMY.REAL_ENTRY_FEE);
const juvenileLand = fmtLt(juvenileLandCents);
const realLand = fmtLt(realLandCents);
const realLandPerGp = (realLandCents / LT_CENTS / ECONOMY.REAL_ENTRY_FEE).toFixed(3);
const juvenileLandPerGp = (juvenileLandCents / LT_CENTS / ECONOMY.JUVENILE_ENTRY_FEE).toFixed(3);
...
//
// ADVANCEMENT splits across every fight won, a round-r win scoring 2^(r-1);
// CHAMPION and RUNNER_UP are bonuses on top. A bird that won w fights holds
// 2^w - 1 of the weight. (A full bracket has no byes, and a bye is not a win,
// so counting forward from the bracket size gives the engine's own answer.)
...
export const MCP_INSTRUCTIONS: string[] = [
  ...
  `CLASSES ARE THE LADDER: open · maiden (never-winners only) · nw3 (fewer than ${NW_CAP} STAKES wins — round 31 merged the old nw2 into it, so there is now ONE conditioned rung between maiden and open) · claimer (priced). The classes NEST — maiden ⊂ nw3 ⊂ open — so a young bird always has a softer class it may drop into, and a veteran always has open. The discovery year runs its OWN, smaller ladder (round 23) — open, maiden, and claimer only; nw3 stays out, since a one-year-old hasn't fought long enough to have the stakes record it sorts by — and its claimer tags are their own cheaper rungs: ${CLAIMER.JUVENILE_PRICES.join("/")} GP, against the grown ladder's ${CLAIMER.PRICES.join("/")} GP. The field is WHOEVER ENTERS — but the board is FOGGED while a lobby is OPEN: lobby_board shows every lobby and its fill count, NEVER whose birds are inside (no dodging — predicting a lobby's strength from its mode, class, and tag is the skill). The one exception: CLAIMER fields are fully visible (stars, records, figures — never stats), because claims are placed on specific birds. Fighting for a tag is choosing to be seen.`,
...
export const TOOL_DESCRIPTIONS = {
  ...
  enter_pintakasi:
    `Register an age-${AGE.FORK}+ bird for one of THIS WEEK's three blade championships (the weekly Majors — crowns every ${DAY_NAMES[PINTAKASI.DAY_OF_WEEK]}). HARDCORE THROUGHOUT: every loser in the bracket is FORCE-RETIRED, so always confirm with the player first. ${PINTAKASI.ENTRY_FEE === 0 ? "FREE to enter" : `${PINTAKASI.ENTRY_FEE} GP to enter`} and OPEN — since round 37 there is NO qualification gate beyond age ${AGE.FORK}+, so a bird with an empty record may register. ⚠ BUT REGISTERING IS NOT STANDING, and you must tell the player that rather than promising them a bracket seat: there are only ${PINTAKASI.MAX_BRACKET} seats, the SELECTION COMMITTEE seats them on CAREER EARNINGS (all the GP the bird has ever won — daily-card pots plus banked purse), and at a full field this call either BUMPS the current weakest bird (refunded, in public) or is REFUSED because your bird is the weakest. A seat won today can also be taken away later in the week by a richer newcomer — nothing is final until ${DAY_NAMES[PINTAKASI.DAY_OF_WEEK]}. Check pintakasi_board for where the bird sits, and if it is near the bump line the fix is to win real fights on the daily card before crown day; the purse = the week's juice pool (less the Juvenile Championship's slice), and since round 40 it is paid on FIGHTS WON, not on finishing place: ${(PINTAKASI.PURSE.ADVANCEMENT * 100).toFixed(0)}% of it splits across every win in the bracket (a win in each round worth double the round before), plus a ${(PINTAKASI.PURSE.CHAMPION * 100).toFixed(0)}% champion and ${(PINTAKASI.PURSE.RUNNER_UP * 100).toFixed(0)}% runner-up bonus. Winning even one Major fight pays (~${majorOneWinPct} of the purse in a full ${majorBracket}-bird bracket; ~${majorChampionPct} for the crown) — a BYE does not, and a bird that wins nothing takes nothing. LAND is weighted to the FALLEN (the earlier a bird goes out, the bigger its land grant — a first-round hardcore death is never a pure loss). Committee-seeded bracket by career earnings → career wins → average figure. One bird per crown and up to ${PINTAKASI.MAX_PER_BARN} birds from your barn per championship; the field is PUBLIC — check pintakasi_board. Registrants fight normal cards all week except ${DAY_NAMES[PINTAKASI.DAY_OF_WEEK]} itself. SAME TOOL ENTERS THE JUVENILE CHAMPIONSHIP: pass division "juvenile" with an age-${AGE.CHICK} bird holding ${JUVENILE_MAJOR.QUALIFYING_WINS} juvenile wins, at ${FORMATS[JUVENILE_MAJOR.BLADES[0]].label} or ${FORMATS[JUVENILE_MAJOR.BLADES[1]].label} (${DAY_NAMES[JUVENILE_MAJOR.DAY_OF_WEEK]}, free, NOT hardcore — nothing is at risk, so you don't need the same solemn confirmation). A bird gets ONE championship a week, so a chick declares for ONE of the two juvenile crowns: send it to the blade its scout report reads better at, since sorting birds onto their end of the dial is what the discovery year is for.`,

  pintakasi_board:
    `This week's three blade championships: each field ranked as the Selection Committee sees it TODAY (rank 1 = top seed; the bottom of a full field is the bump line), the entry fee, and the projected purse (entries so far + the juice-pool share). Fields are PUBLIC — the Pintakasi is the one un-fogged stage in the game. Pass division "juvenile" for ${DAY_NAMES[JUVENILE_MAJOR.DAY_OF_WEEK]}'s two discovery-year crowns instead (${FORMATS[JUVENILE_MAJOR.BLADES[0]].label} and ${FORMATS[JUVENILE_MAJOR.BLADES[1]].label}) — read both fields before you declare a chick for one, since it may only stand in one a week.`,
