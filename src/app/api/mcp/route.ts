...
const juvenileFightLand = fmtLt(landPotShare(JUVENILE_MAJOR.LAND_POT, slotsIn(juvenileBracket), 1));
...
export const MCP_INSTRUCTIONS: string[] = [
  ...
  "ONE LIFETIME RECORD (ruled round 15): juvenile, real, and hardcore fights ALL count toward the same wins-losses line, for birds and farms alike. NOTE: the record does NOT set stud prices — stud pricing is player speculation and supply/demand (flat " +
    ECONOMY.BREED_FEE +
    " GP for now).",
...
export const TOOL_DESCRIPTIONS = {
  ...
  breed: `Buy a cover: YOUR retired hen × a retired rooster — your own, or ANY farm's listed stud (browse_studs first). Costs ${ECONOMY.BREED_FEE} GP ($${usd(ECONOMY.BREED_FEE)}, min AND max for now), which SPLITS: ${fmtGp(breedSplit.stakerPoolCents)} GP to the land-staking pool, ${fmtGp(breedSplit.juicePoolCents)} to the fight-juice pool, ${fmtGp(breedSplit.studOwnerCents)} to the stud's owner. The hen's farm keeps the egg ('Egg of <mother>'). She is pregnant until the next Friday, then the egg lays and she is free for another cover; that egg hatches the Friday after. Covers are capped per rooster per week (${COVERS.PER_WEEK} public + ${COVERS.OWNER_RESERVED} owner-reserved).`,

  browse_studs: `The barn from one hen's point of view: every stud she CAN breed with (name, farm, stars, age, record, covers left, AND his full revealed six-stat sheet + overall grade — he is retired, so nothing about him is fogged; ${ECONOMY.BREED_FEE} GP each) plus the excluded ones WITH the reason (kin overlap named explicitly, or covered out this week). Candidates = every farm's listed studs + your own retired roosters.`,

  list_stud: `List a retired rooster of yours in the breeding barn — any farm's hens can then buy covers at ${ECONOMY.BREED_FEE} GP, of which ${fmtGp(breedSplit.studOwnerCents)} GP lands in YOUR wallet per cover. ${COVERS.PER_WEEK} public covers/week plus ${COVERS.OWNER_RESERVED} reserved for your own hens. THE FIRST TIME you open a rooster's public slots it costs ${studSeatLand} LT, spent outright — not staked, not refundable; pulling him and re-listing him later is free, the land buys the seat once, not a subscription. Selling covers is real income — list your good retirees.`,
...
function createServer(farmId: string | null): McpServer {
    ...
    async (input) =>
      ruled(() => {
        ...
        return {
          farm,
          apiKey,
          note: "Save the apiKey — it is your login (?key=… on the MCP URL).",
        };
      })
  );
  ...
  server.registerTool(
    ...
    async ({ amount }) =>
      ruled(() => {
        const g = game();
        return g.farms.stake(g.farmId, amount);
      })
  );
  ...
  server.registerTool(
    ...
    async ({ amount }) =>
      ruled(() => {
        const g = game();
        return g.farms.unstake(g.farmId, amount);
      })
  );
  ...
  server.registerTool(
    ...
    async () =>
      ruled(() => {
        const g = game();
        return g.farms.expandBarn(g.farmId);
      })
  );
  ...
  server.registerTool(
    "enter_lobby",
    {
      ...
      inputSchema: z.object({
        ...
        format: z
          ...
          .default("b2")
          .describe(
            "b1 = sprint · b2 = hybrid · b3 = route · b4 = marathon · b5 = classic — only the blades on today's card are enterable"
          ),
        classType: z
          .enum(ENTRY_CLASSES)
          .default("open")
    ...
    async ({ birdId, mode, format, classType }) =>
      ruled(() =>
        game().lobbies.enter(birdId, {
          mode,
          classType: classType as never,
    ...
    "enter_claimer",
    {
      ...
      inputSchema: z.object({
        ...
        format: z
          ...
          .default("b2")
          .describe(
            "The weapon format the lobby runs at — must be one today's card posted for a claimer"
          ),
        price: z
          .number()
          .int()
