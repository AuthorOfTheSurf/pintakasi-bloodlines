...
export const MCP_INSTRUCTIONS: string[] = [
  ...
  "EVERY FIGHT IS PvP — PURE, BETWEEN BARNS. The house supplies nobody. The rhythm: during the game-day you ENTER birds into lobbies (enter_lobby / enter_claimer); at the day tick every lobby GOES OFF — its birds are randomly paired and fight each other. Matchmaking NEVER pairs two birds from the same barn: enter several birds in one lobby freely, they will only ever draw other farms (birds left with only barn-mates go unmatched and refund). Entries are BINDING (fee escrowed, the bird's daily fight spent). LOBBIES ARE UNBOUNDED (round 31): there is exactly ONE lobby per posted fight per day and it grows without limit, so there is no room to be shut out of and nothing to gain by camping — entering early and entering late land you in the same field. What a big lobby costs you is certainty: the pairing is random, so a deep field is a deeper lottery over who you draw. A lobby that goes off with an odd number of birds strands one, whose fee refunds.",
  ...
  `THE ECONOMY IS POOLED ($1 = ${ECONOMY.GP_PER_DOLLAR} GP): both sides post the entry. ` +
    ...
    ` No fight ever prints GP. Every REAL win also banks PINTAKASI QUALIFICATION POINTS (+${PINTAKASI.POINTS_FOR.real} a win; the discovery year pays ${PINTAKASI.POINTS_FOR.juvenile}) — that is how a bird earns its way into a championship, and since round 31 took hardcore off the daily card it is the ONLY way: winning a crown banks no points at all. The subsidy is LAND: both fighters earn Land Tokens scaled to the entry fee and slightly MORE than linearly (juvenile ${juvenileLand} LT · real ${realLand} LT; tournament fights mint on their own, steeper curve) — fighting UP into dearer company pays extra land. Unmatched birds earn none. Land is also buyable (buy_land, $${usd(LAND.GP_PER_100_TOKENS)}/LT, capped at ${LAND.DAILY_BUY_CAP.toLocaleString()} LT/game-day) and NEVER sellable.`,
...
export const TOOL_DESCRIPTIONS = {
  ...
  enter_lobby:
    `Put a bird on tonight's card — PURE PvP: at the day tick the lobby's birds are randomly paired and fight EACH OTHER (never two of your own — enter several birds freely, matchmaking keeps barn-mates apart). ⚠ THE FIGHT MUST BE ON TONIGHT'S CARD: only ~${cardKeysPerDay} (mode, class, blade) combinations are posted each day, and this tool's dials list every LEGAL value, not today's available ones — so read get_state's \`card\` or lobby_board FIRST and enter one of the keys you find there. Anything else is refused with "that fight isn't on tonight's card", and nothing is charged. BINDING once accepted: the fee escrows and the bird's daily fight is spent. Lobbies have no size limit — join a busy one freely; birds left without an opponent refund. Pick the WEAPON FORMAT (distance dial) and CLASS (ladder dial) deliberately from what is posted — lobby_board shows fill counts, not fields (fogged), so judge where your bird belongs. Modes: juvenile (age ${AGE.CHICK} only, ${ECONOMY.JUVENILE_ENTRY_FEE} GP) · real (${AGE.REAL_STAKES}+, ${ECONOMY.REAL_ENTRY_FEE} GP) — both feed the ONE lifetime record. There is NO hardcore mode any more (round 31): nothing entered here can end a career; hardcore lives only in the Majors, via enter_pintakasi. Land pays both fighters, scaled up with the fee. Claimers run through enter_claimer.`,
