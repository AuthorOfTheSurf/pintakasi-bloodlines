...
export const TOOL_DESCRIPTIONS = {
  ...
  list_flock:
    "Every bird in YOUR barn with derived age, element stars (e.g. '2.5★ Fire'), carriage, record, and status (egg/active/retired). THE FOG (round 28): the six fighting stats read null while a bird can still fight — they REVEAL at retirement. A live bird is judged by its figures, not a sheet. Retired roosters show whether they're standing at stud (listedStud).",

  get_bird:
    "One bird in full: carriage, lineage tree, and the DISCOVERY READOUT — `scoutReport` (all five blades: record, average and best Pit Figure, and a shrunk `score` per blade with `bestBlade` flagged; unraced blades score the neutral prior, so one loud figure never types a bird), the raw per-format lines (`formatRecords`), and `formBook`: every past fight stamped with that day's ascendant Element, `edge: true` marking fights run in its OWN weather (read those figures down a touch), plus `onEdge`/`offEdge` averages. THE FOG (round 28): the six stats are null while the bird can fight and REVEAL at retirement — on a retired bird this is the full sheet.",
