...
**AI barns and Rivet Actors have their own ledger** — `BRAINS.md` at the root: the inference-side speed numbers, the design findings, and the phase-2 actor notes. **Experiments archive to `runs/`** (one date-stamped directory each; `journal.md` is the front door, sim logs stay pure, coach sessions file alongside — see `runs/README.md`). The short operational version:
...
- `bun run tune <farm> "<standing orders>"` writes strategy into a barn actor's durable state; the barn folds it into its next morning's prompt. **Tune live worlds** (mid-run binds first try); tuning a cold barn after its sim exits walks into the rebind window and, under an aged daemon, the no-rebind bug. `--clear` removes orders; bare `tune <farm>` shows career + orders.
- `--personas` (with `--actors`) starts each llm barn under its scripted twin's house creed — goals port over, decision logic does not (`src/actors/personas.ts`). The coaching loop — a big model reading `brain_log` and writing orders via tune — is documented in `COACHING.md`.
- Full fleet (19 llm barns) works: ~103 s/game-day on qwen3:14b, **~51 s/game-day on qwen3:30b-a3b** (MoE — twice the disk, half the wall clock; prefer it for fleet runs). The decider timeout scales with fleet size in simulate.ts — at 19 barns most of a call is Ollama's QUEUE, not the model thinking, and a flat 120s timed out 6 of 19.
- Two fleet-scale failure modes are fixed in barn.ts and should stay fixed: `maxIncomingMessageSize: 8 MB` on the registry (a 100-bird barn's raw view is ~100 KB against a 64 KB default — retry-proof, the barn just vanishes from every day) and `noSleep: true` on the actor (all 19 barns waking at once each morning on an Ollama-pinned machine misses the engine's 5 s wake deadline and collapses the fleet).
...
- **Style cleanups are behaviour-neutral, and that gets proven.** Any refactor that touches the engine gets the 92-day `worldhash` identity check below before it commits.
